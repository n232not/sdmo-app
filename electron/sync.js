...
const INLINE_REVIEWS_MAX_BYTES = 5 * 1024 * 1024
...
function buildProjectStateIndexExport(db, projectId) {
  ...
  const inlineReviewBytes = Buffer.byteLength(JSON.stringify(reviews), 'utf8')
  const inlineReviews = inlineReviewBytes <= INLINE_REVIEWS_MAX_BYTES
  return {
    sdmo_sync: true,
    protocol_version: SYNC_PROTOCOL_VERSION,
    layout: 'hybrid-v1',
    ...
    exported_at: new Date().toISOString(),
    review_storage: {
      mode: inlineReviews ? 'inline' : 'split',
      max_inline_bytes: INLINE_REVIEWS_MAX_BYTES,
      inline_bytes: inlineReviewBytes,
    },
    project: config.project,
    forms: config.forms || [],
    form_versions: formPayloads,
    instructions: config.instructions || [],
    media_types: config.media_types || [],
    media_type_versions: mediaTypePayloads,
    encounters: config.encounters || [],
    reviews: inlineReviews ? reviews : reviews.map(reviewIndexEntry),
    deleted_structure: buildStructureTombstones(db, projectId),
  }
}
...
function remoteReviewsNeedingHydration(remoteIndex, localIndex) {
  if (!usesSplitReviewPayloads(remoteIndex) || !localIndex) return remoteIndex?.reviews || []
...
function usesSplitReviewPayloads(stateData) {
  if (!stateData?.sdmo_sync) return false
  if (stateData.layout === 'split-v1') return true
  return stateData.review_storage?.mode === 'split'
}
...
function hydrateSplitProjectStateLocal(stateData, syncFolder, { localIndex = null } = {}) {
  ...
  state.form_versions = (stateData.form_versions || []).filter(v => !v.current)
  ...
  if (usesSplitReviewPayloads(stateData)) {
    ...
    state.reviews = (stateData.reviews || []).map(review => expandReviewFromSync(review, formCatalog))
  }
  return state
}
...
function writeSplitProjectStateLocal(db, projectId, syncFolder, indexState = buildProjectStateIndexExport(db, projectId), previousIndex = null) {
  ...
  if (usesSplitReviewPayloads(indexState)) fs.mkdirSync(reviewsDir, { recursive: true })
  ...
  if (usesSplitReviewPayloads(indexState)) {
    for (const review of buildReviewStateExport(db, projectId).map(r => compactReviewForSync(r, formCatalog))) {
      writePayload(reviewsDir, reviewIndexEntry(review), previousReviewHashes, review)
    }
  }
...
async function writeSplitProjectStateCloud(db, projectId, adapter, folderId, indexState = buildProjectStateIndexExport(db, projectId), previousIndex = null) {
  ...
  if (usesSplitReviewPayloads(indexState)) {
    reviewsDirId = await adapter.ensureFolder(folderId, REVIEWS_DIR)
    const reviewFiles = await adapter.listFiles(reviewsDirId)
    existing = { [REVIEWS_DIR]: new Set(reviewFiles.map(f => f.name)) }
  }
  const config = buildConfigExport(db, projectId)
  const formPayloads = buildFormVersionPayloads(db, projectId, config)
  const formCatalog = new Map(formPayloads.map(v => [formVersionKey(v.form_sync_id, v.version), v]))
...
function syncProjectStateLocal(db, projectId, syncFolder) {
  ...
  if (fs.existsSync(statePath)) {
    remoteIndex = JSON.parse(fs.readFileSync(statePath, 'utf8'))
    const stateData = hydrateSplitProjectStateLocal(remoteIndex, syncFolder, { localIndex })
  ...
  if (postFingerprint !== folderFingerprint || !fs.existsSync(statePath) || !isProtocolV2Manifest(manifest)) {
    writeSplitProjectStateLocal(db, projectId, syncFolder, postIndex, remoteIndex)
  }
  return { conflicts }
}
